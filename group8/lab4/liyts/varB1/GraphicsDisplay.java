...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(3.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_MITER, 10.0f, new float[]{10f, 5f, 3f, 5f, 3f, 5f, 7f, 5f, 7f, 14f}, 0.0f);
...
// Шаг 2 - Организовать цикл по всем точкам графика
        for (Double[] point : graphicsData) {
            ...
            double r = 5; // половина 11px (11x11 → ±5 от центра)

            // Горизонтальная линия
            canvas.draw(new Line2D.Double(cx - r, cy, cx + r, cy));
            // Вертикальная линия
            canvas.draw(new Line2D.Double(cx, cy - r, cx, cy + r));
            // Диагональ ↘↖
            canvas.draw(new Line2D.Double(cx - r, cy - r, cx + r, cy + r));
            // Диагональ ↗↙
            canvas.draw(new Line2D.Double(cx - r, cy + r, cx + r, cy - r));
        }
    }
